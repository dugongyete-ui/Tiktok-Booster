...
class ProgramUsage():
    ...
    def get_vmid(VIDEO):
        ...
        try:
            response = requests.post(
                "https://countik.com/api/video/exist",
                json={'url': f"{VIDEO}"},
                timeout=10
            ).json()
            return response['id']
            ...
            print(f"{Fore.RED}[Warning] get_vmid error: {e}{Style.RESET_ALL}")
            return None
    ...
    def convert_hours(hours='ind', sec='ind'):
        ...
        Returns
        -------
        - str : the hhmmss based on the parameter passed.
        """
        if hours == 'ind':
                    ...
                    if entry.is_dir():
                        with os.scandir(entry) as entries_folder:
                            for entry_folder in entries_folder:
                                try:
                                    os.replace(
                                        f"{os.curdir}/Tiktok-Booster-Update/{entry.name}/{entry_folder.name}",
    ...
    def Activate(sha256_hash, file_path, UUID):
        try:
            response = requests.get(
                f"https://sneezedip.pythonanywhere.com/get_key?uuid={UUID}", timeout=10
            ).json()
            key_text = response.get('response', 'N/A')
        except Exception:
            key_text = 'N/A (server unavailable)'
...
{Fore.YELLOW}Please join https://discord.gg/nAa5PyxubF and go to the \'get-key\' channel\n {Style.RESET_ALL}''')
        print(f'{Style.BRIGHT}YOUR KEY : {Fore.LIGHTGREEN_EX}{key_text}{Style.RESET_ALL}')
        try:
        ...
        while True:
            ...
            try:
                validate_response = requests.get(
                    f"https://sneezedip.pythonanywhere.com/validate_activation?uuid={UUID}&key={activation}",
                    timeout=10
                )
                validate_json = validate_response.json()
            except Exception:
                print(f"{Fore.RED}Could not reach activation server. Please try again.{Style.RESET_ALL}")
                continue
            spinner = Halo(text="Verifying key", spinner="dots")
            spinner.start()
            time.sleep(2)
            if 'Valid Key!' in validate_json.get('response', ''):
    ...
    def get_history():
        try:
            ...
            return None
        history = [info for info in history_data["history"]]
    ...
    def Translations(section, id):
        ...
        if ProgramUsage._trans_lang != lang:
            ProgramUsage._trans_lang = lang
            ProgramUsage._trans_cache = {}
            with open("languages.json", "r", encoding="utf-8") as file:
                data = json.loads(file.read())
            translation_language = data["translations"][lang]
            for item_section in ["credits", "updates", "history", "errors", "main"]:
                for item in translation_language[0].get(item_section, []):
                    ProgramUsage._trans_cache[f"{item_section}:{item['id']}"] = item["text"]

        return ProgramUsage._trans_cache.get(f"{section}:{id}", "")
    ...
    def is_down():
        ...
        try:
            return requests.get("https://zefoy.com/", headers=headers, timeout=10).status_code != 200
        except Exception:
            return True
