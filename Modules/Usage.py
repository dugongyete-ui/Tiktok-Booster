...
class ProgramUsage():
    ...
    def Activate(sha256_hash,file_path,UUID):
        try:
            response = requests.get(f"https://sneezedip.pythonanywhere.com/get_key?uuid={UUID}", timeout=10).json()
            key_text = response.get('response', 'N/A')
        except Exception:
            key_text = 'N/A (server unavailable)'
            response = {'response': key_text}
...
{Fore.YELLOW}Please join https://discord.gg/nAa5PyxubF and go to the \'get-key\' channel\n {Style.RESET_ALL}''')
        ...
        while True:
            ...
            try:
                validate_response = requests.get(f"https://sneezedip.pythonanywhere.com/validate_activation?uuid={UUID}&key={activation}", timeout=10)
